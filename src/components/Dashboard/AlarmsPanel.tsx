...
export function AlarmsPanel({ alarms, now, onAcknowledge }: AlarmsPanelProps) {
  const open = alarms.filter(isOpen).slice().reverse();
  ...
  return (
    ...
    >
      ...
      {open.length === 0 ? (
        <p className="px-3 py-6 text-center text-xs text-hmi-muted">
          No active alarms. Fleet within limits.
        </p>
      ) : (
